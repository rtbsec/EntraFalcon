...
"@
    ...
    $ChartsectionManagedIdentities += New-ChartSection -Title "Managed Identities" -Prefix "managedidentities" -ChartCount 4
    ...
    $ChartsectionAzureRoles += New-ChartSection -Title "Azure Role Assignments" -Prefix "azureroles" -ChartCount 6
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const dataSources = {
        ...
        managedidentities_azurelevels: {
            'Critical': $([int]$GlobalAuditSummary.ManagedIdentities.AzureImpactLevel.Critical),
            'High': $([int]$GlobalAuditSummary.ManagedIdentities.AzureImpactLevel.High),
            'Medium': $([int]$GlobalAuditSummary.ManagedIdentities.AzureImpactLevel.Medium),
            'Low': $([int]$GlobalAuditSummary.ManagedIdentities.AzureImpactLevel.Low),
            'Unknown': $([int]$GlobalAuditSummary.ManagedIdentities.AzureImpactLevel.Unknown)
        },
        ...
        azureroles_levels: {
            'Critical': $([int]$GlobalAuditSummary.AzureRoleAssignments.Levels.Critical),
            'High': $([int]$GlobalAuditSummary.AzureRoleAssignments.Levels.High),
            'Medium': $([int]$GlobalAuditSummary.AzureRoleAssignments.Levels.Medium),
            'Low': $([int]$GlobalAuditSummary.AzureRoleAssignments.Levels.Low),
            'Unknown': $([int]$GlobalAuditSummary.AzureRoleAssignments.Levels.Unknown)
        },
        ...
        azureroles_scopetypes: {
            'Root': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.Root),
            'Management Group': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.ManagementGroup),
            'Subscription': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.Subscription),
            'Resource Group': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.ResourceGroup),
            'Resource': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.Resource),
            'Unknown': $([int]$GlobalAuditSummary.AzureRoleAssignments.ScopeType.Unknown)
        }        
    };
    ...
    function getChartData(datasetKey) {
        ...
        if (datasetKey === 'azureroles_levels') {
            const entries = Object.entries(dataSources.azureroles_levels).filter(e => e[0] !== 'Unknown' || e[1] > 0);
            return {
                labels: entries.map(e => e[0]),
                datasets: [{
                    label: 'Assignments',
        ...
        if (datasetKey === 'managedidentities_azurelevels') {
            const entries = Object.entries(dataSources.managedidentities_azurelevels).filter(e => e[0] !== 'Unknown' || e[1] > 0);
            return {
                labels: entries.map(e => e[0]),
                datasets: [{
                    label: 'Assignments',
        ...
        return {
            ...
            options: {
                ...
                scales: type === 'bar' ? {
                    x: {
                        display: true,
                        ticks: indexAxis === 'y' ? { color: axisColor, precision: 0 } : { color: axisColor },
                        grid: { color: gridColor }
                    },
                    y: {
                        display: true,
                        ticks: indexAxis === 'y' ? { color: axisColor } : { color: axisColor, precision: 0 },
                        grid: { color: gridColor }
                    }
                } : {}
            },
    ...
    const chartConfigs = [
        ...
        { id: 'managedidentities_chart2', title: 'Direct Azure Assignments by Scope Level', type: 'bar', dataset: 'managedidentities_azurescopetypes', indexAxis: 'y', showLegend: false },
        { id: 'managedidentities_chart3', title: 'Direct Azure Assignments by Impact Level', type: 'bar', dataset: 'managedidentities_azurelevels', indexAxis: 'y', showLegend: false },
        { id: 'managedidentities_chart4', title: 'API Permission Severity', type: 'bar', dataset: 'managedidentities_apicategorization', indexAxis: 'y', showLegend: false },
        ...
        { id: 'azureroles_chart4', title: 'Assignments by Scope Level', type: 'bar', dataset: 'azureroles_scopetypes', indexAxis: 'y', showLegend: false },
        { id: 'azureroles_chart5', title: 'Assignments by Impact Level', type: 'bar', dataset: 'azureroles_levels', indexAxis: 'y', showLegend: false },
        { id: 'azureroles_chart6', title: 'Role Tier Distribution', type: 'bar', dataset: 'azureroles_tiers', indexAxis: 'y', showLegend: false }
    ];
